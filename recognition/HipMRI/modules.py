...
class SE3D(nn.Module):
    """3D Squeeze-and-Excitation: channel-wise reweighting via global pooling and 1x1 conv MLP."""
...
class Res3D(nn.Module):
    """3D residual block with GroupNorm, SiLU, optional Dropout, and optional SE attention."""
...
class ASPP3D(nn.Module):
    """3D Atrous Spatial Pyramid Pooling: multi-dilation features + image-level pooling, then projection."""
...
class AttnGate3D(nn.Module):
    """Additive attention gate for skip connections, conditioned on decoder features."""
    ...
    def forward(self, x_skip, g):
        ...
        if t.shape[-3:] != p.shape[-3:]:
            p = F.interpolate(
                p, size=t.shape[-3:], mode="trilinear", align_corners=False
            )
...
class Up3D(nn.Module):
    """Decoder upsampling block: upsample + attentional skip fusion + residual refinement."""
    ...
    def __init__(self, in_ch, skip_ch, out_ch, dropout=0.0, groups=16, use_se=True):
        ...
        self.res = Res3D(
            out_ch + skip_ch, out_ch, dropout=dropout, groups=groups, use_se=use_se
        )
...
class ImprovedUNet3D(nn.Module):
    """UNet-like 3D segmentation network with Res3D encoders, ASPP bottleneck, attention-gated skips, and SE."""
...
class DiceLoss3D(nn.Module):
    """Multi-class soft Dice loss over 3D volumes; ignores classes absent from both pred and target."""
