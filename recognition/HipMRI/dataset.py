...
def find_pairs(img_dir: str, lab_dir: str) -> List[Tuple[str, str, str]]:
    """Find matching MRI image–label file pairs (LFOV ↔ SEMANTIC) and return list of (key, img_path, lab_path)."""
    ...
    print(f"Found {len(pairs)} 3D pairs.")
    return pairs
...
def pad_or_crop_center(vol: np.ndarray, target, pad_val=0) -> np.ndarray:
    """Center pad or crop a 3D volume to a target shape."""
    z, y, x = vol.shape
    tz, ty, tx = target
    ...
    if pz or py or px:
        vol = np.pad(
            vol,
            ((pz // 2, pz - pz // 2), (py // 2, py - py // 2), (px // 2, px - px // 2)),
            mode="constant",
...
class HipMRI3DDataset(Dataset):
    """Custom PyTorch Dataset for 3D hip MRI images and segmentation labels,
    handles normalization, resizing, and class inference."""

    def __init__(
        self, pairs: List[Tuple[str, str, str]], patch_size=(256, 256, 128), norm=True
    ):
        self.items = pairs
        self.ps = patch_size
        self.norm = norm
        ...
        print(f"Detected num_classes={self.num_classes} (from sample labels).")
